...
class PartnersTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = '.Map.PartnersTableMap';
     ...
     */
    const OM_CLASS = '\\Partners';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'Partners';
     ...
     */
    const COL_PATIENT_ID = 'partners.patient_id';

    /**
     * the column name for the created_at field
     */
    const COL_CREATED_AT = 'partners.created_at';

    /**
     * the column name for the updated_at field
     */
    const COL_UPDATED_AT = 'partners.updated_at';
  ...
  array (
     ...
     * and findPk*() calls.
     *
     * @param \Partners $obj A \Partners object.
     ...
     *
     * @param mixed $value A \Partners object or a primary key value.
     */
    public static function removeInstanceFromPool($value)
    {
        if (Propel::isInstancePoolingEnabled() && null !== $value) {
            if (is_object($value) && $value instanceof \Partners) {
                ...
                $e = new PropelException("Invalid value passed to removeInstanceFromPool().  Expected primary key or \Partners object; got " . (is_object($value) ? get_class($value) . ' object.' : var_export($value, true)));
                throw $e;
            }
    ...
    public static function getPrimaryKeyFromRow($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        ...
        $pks[] = (int) $row[
            $indexType == TableMap::TYPE_NUM
                ? 1 + $offset
                : self::translateFieldName('PatientId', TableMap::TYPE_PHPNAME, $indexType)
        ];
    ...
    {
        return $withPrefix ? PartnersTableMap::CLASS_DEFAULT : PartnersTableMap::OM_CLASS;
    }
     ...
     * @return array           (Partners object, last column rank)
     */
    public static function populateObject($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        $key = PartnersTableMap::getPrimaryKeyHashFromRow($row, $offset, $indexType);
        if (null !== ($obj = PartnersTableMap::getInstanceFromPool($key))) {
            // We no longer rehydrate the object, since this can cause data loss.
            // See http://www.propelorm.org/ticket/509
            // $obj->hydrate($row, $offset, true); // rehydrate
            $col = $offset + PartnersTableMap::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = PartnersTableMap::OM_CLASS;
            /** @var Partners $obj */
            $obj = new $cls();
            $col = $obj->hydrate($row, $offset, false, $indexType);
            PartnersTableMap::addInstanceToPool($obj, $key);
        }
    ...
    public static function populateObjects(DataFetcherInterface $dataFetcher)
    {
        ...
        while ($row = $dataFetcher->fetch()) {
            $key = PartnersTableMap::getPrimaryKeyHashFromRow($row, 0, $dataFetcher->getIndexType());
            if (null !== ($obj = PartnersTableMap::getInstanceFromPool($key))) {
                // We no longer rehydrate the object, since this can cause data loss.
                // See http://www.propelorm.org/ticket/509
    ...
    {
        return Propel::getServiceContainer()->getDatabaseMap(PartnersTableMap::DATABASE_NAME)->getTable(PartnersTableMap::TABLE_NAME);
    }
    ...
    {
        $dbMap = Propel::getServiceContainer()->getDatabaseMap(PartnersTableMap::DATABASE_NAME);
        if (!$dbMap->hasTable(PartnersTableMap::TABLE_NAME)) {
            $dbMap->addTableObject(new PartnersTableMap());
        }
    }

    /**
     * Performs a DELETE on the database, given a Partners or Criteria object OR a primary key value.
     *
     * @param mixed               $values Criteria or Partners object or primary key or array of primary keys
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \Partners) { // it's a model object
        ...
        $query = PartnersQuery::create()->mergeWith($criteria);

        if ($values instanceof Criteria) {
            PartnersTableMap::clearInstancePool();
        } elseif (!is_object($values)) { // it's a primary key, or an array of pks
            foreach ((array) $values as $singleval) {
                PartnersTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    {
        return PartnersQuery::create()->doDeleteAll($con);
    }

    /**
     * Performs an INSERT on the database, given a Partners or Criteria object.
     *
     * @param mixed               $criteria Criteria or Partners object containing data that is used to create the INSERT statement.
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria instanceof Criteria) {
            ...
            $criteria = $criteria->buildCriteria(); // build Criteria from Partners object
        }
...
//
PartnersTableMap::buildTableMap();
